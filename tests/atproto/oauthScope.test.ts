...
import { SCHEDULE_NSID, RESPONSE_NSID, EVENT_NSID, AVAILABILITY_NSID } from '../../src/atproto/records.js';
...
describe('OAuth scope', () => {
  it('asks for the record types the app writes, and never the catch-all', () => {
    ...
    for (const nsid of [SCHEDULE_NSID, RESPONSE_NSID, EVENT_NSID, AVAILABILITY_NSID]) {
      expect(scopes.some((s) => s === `repo:${nsid}` || s.startsWith(`repo:${nsid}?`))).toBe(true);
    }
    // Four collections, four grants: a new write path must show up here too.
    expect(scopes.filter((s) => s.startsWith('repo:'))).toHaveLength(4);
    // Nothing wildcarded.
