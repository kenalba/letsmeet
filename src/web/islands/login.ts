/**
 * Handle typeahead for the sign-in form. Progressive enhancement over a plain text
 * input: without this script the form is exactly what it was. With it, the input becomes
 * an ARIA combobox whose suggestions come from our own `/api/handles` (see
 * handleSearch.ts) — debounced, cancellable, keyboard-navigable.
 */
...
function mount(input: HTMLInputElement): void {
  ...
  list.id = 'handle-suggestions';
  list.setAttribute('role', 'listbox');
  list.setAttribute('aria-label', 'Matching handles');
  list.hidden = true;
  list.className = 'absolute left-0 right-0 top-full z-10 mt-1 max-h-72 overflow-auto rounded-md border bg-popover p-1 text-popover-foreground shadow-md';
...
if (el instanceof HTMLInputElement) mount(el);
