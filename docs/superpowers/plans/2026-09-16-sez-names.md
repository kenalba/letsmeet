...
describe('fallbackHandles', () => {
  ...
  it('is empty for a label with no hyphen (a handle needs two labels) and for one with too many', () => {
    ...
    expect(fallbackHandles('a-b-c-d-e')).toEqual([]); // 5 hyphens: 32 readings > MAX_FALLBACK_CANDIDATES
    expect(fallbackHandles('a-b-c-d').length).toBe(15); // 4 hyphens: every reading but "no dots"
...
server {
    ...
    listen [::]:443 ssl;
    listen 443 ssl;
    ssl_certificate /etc/letsencrypt/live/sez.letsmeet.lol/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/sez.letsmeet.lol/privkey.pem;
...
- **Certificate: `*.sez.letsmeet.lol` by DNS-01.** Wildcards cannot use HTTP-01;
  ...
  zone, which lives at Marque (mqdns). Marque hands out two certbot hook scripts
  and a dedicated `MARQUE_DNS01_TOKEN` for exactly this. On the box, as root:

  ```bash
  install -d -m 700 /etc/letsencrypt/marque
  # paste Marque's auth and cleanup hook scripts:
  install -m 700 /dev/stdin /etc/letsencrypt/marque/auth.sh     # then paste, ctrl-d
  install -m 700 /dev/stdin /etc/letsencrypt/marque/cleanup.sh  # then paste, ctrl-d
  # the token, and nothing else, in a root-only env file:
  ( umask 077; printf 'MARQUE_DNS01_TOKEN=%s\n' '<token>' > /etc/letsencrypt/marque.env )
  ```

  Each hook must read the token from that file; if Marque's scripts expect it
  in the environment, add `set -a; . /etc/letsencrypt/marque.env; set +a` as
  their second line. Certbot passes `CERTBOT_DOMAIN` and `CERTBOT_VALIDATION`
  to the auth hook and the same plus `CERTBOT_AUTH_OUTPUT` to the cleanup hook.

  Dry run first, then for real; the `--cert-name` is what the nginx block
  above refers to under `/etc/letsencrypt/live/`:

  ```bash
  sudo certbot certonly --dry-run --manual --preferred-challenges dns \
    --manual-auth-hook /etc/letsencrypt/marque/auth.sh \
    --manual-cleanup-hook /etc/letsencrypt/marque/cleanup.sh \
    --cert-name sez.letsmeet.lol -d '*.sez.letsmeet.lol'
  ...
  Renewal is certbot's normal timer: the hooks and the deploy hook are recorded
  in `/etc/letsencrypt/renewal/sez.letsmeet.lol.conf`, so `certbot renew` runs
  them again. Confirm with `sudo certbot renew --dry-run`.
