...
| Notes | Live **inside the away zone**: a label laid over the entry's own rectangle, vertical down a one-day zone, horizontal and wrapping across a multi-day span, clipped to the zone. Zones under three hours tall carry no label. Headers are number and weekday only; the friend view's `<small>` note under the header goes. |
| Adding a note | Marking away drops a **chip** under the marked cells: "away sat 19 11am–3pm" and a note field with an "add" button. Enter or "add" saves; Escape or the next mark dismisses. Focus goes to the field on mouse; on touch the chip waits for a tap so the keyboard does not jump. |
...
| Copy feed link | Reads `copy link`, right-aligned in the links row on both pages (it is a button; the anchors stay left), short enough that `download .ics · subscribe (webcal)` and the button share one line at phone width. The stray `·` before it goes. |
...
- Links row: `download .ics · subscribe (webcal)` left, `copy link` right (`margin-left: auto`). Caption: `click a free hour to ping on bluesky.`
