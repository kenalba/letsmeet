...
| Autosave | Every change posts about two seconds after the last one: grid strokes on pointer-up, header taps, chip and list notes, remove, timezone, good-through, note for friends, sez name. Text fields also post on blur. Never mid-stroke. Leaving the page (`pagehide`, or the tab going hidden) flushes a pending save at once with `keepalive`. The server's identical-write skip stays, so a no-op edit writes nothing. |
