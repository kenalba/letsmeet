...
- **OAuth scope is narrow** (since 2026-09-01): `atproto` plus `repo:` grants for exactly the three collections the app writes — `lol.letsmeet.poll.schedule` (create/update/delete), `lol.letsmeet.poll.response` (create/update) and `community.lexicon.calendar.event` (create); see `SCOPE` in `src/atproto/oauthClient.ts` and the test that pins it. Earlier builds asked for `transition:generic`, whose consent screen reads "manage your profile, posts, likes and follows" and "any public data"; a responder declined it, rightly. Sessions granted before the change keep the old scope until their next sign-in. A new write path needs its collection added to `SCOPE`, or the PDS will refuse the write.
